...
The rail renders from the hub's registry (`config/glacier-pack.php`), which lists all 46 addons. For each entry the hub checks whether the addon's pane partial exists on the panel:
...
- **[Addons Guide →](./addons.md)** — what each of the 46 addons does and where it lives in the hub.
