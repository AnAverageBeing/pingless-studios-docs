...
- **`attacks`** — up to 50 recent attack records: peak/avg/p95 pps, IPs involved, protocol mix, `forensics_dir`. Each record's `top_targets` entries carry `state` (per-tenant attack state) and, since v2.22 build 3, **`scope`** — the network-ownership classification from the attached-IP registry: `pool` (operator-declared CIDR pool), `manual` (operator-added entry), `auto` (wire-learned entry), or `transit` (unknown). On forwarding deployments (VXLAN, routed upstream) dst_stats sees every destination crossing the wire, and only `pool`/`manual` means "this network is ours" — declare your allocations (`openshield ips add` / pool add) so end-of-attack reports split "your networks" from learned/transit destinations; with nothing declared, all targets read `auto` and alerts render the legacy flat list.
